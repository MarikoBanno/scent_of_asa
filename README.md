...
現状は「公開予約フロントはほぼ完了、バックエンドを順次接続している段階」です。
...
デフォルトで木曜と金曜日が営業日に設定されています。所定の日付を設定したい場合は以下のとおりしてみてください。
...
## スタッフサイドの操作をする
ログインする
https://scent-of-asa.onrender.com/staff/login.html
（ID・パスワード不要）

○　休業日を設定する
https://scent-of-asa.onrender.com/staff/holiday_control.html
デフォルトですべて営業日になっているので、
決まった曜日を休みにする場合は、画面右のパネル「定休日ルール」から曜日を選択し「今月に一括適用」をクリックする。
特定の1日を休みにする場合は、カレンダーから日付をクリックし、右パネル選択日でステータスを選択して「この日の設定を保存」する。（英語/日本語分けて設定できます。）

○　枠を管理する
https://scent-of-asa.onrender.com/staff/slot.html
デフォルトで木金を設定しています。
過去日は操作できません。
カレンダーの日付をクリックすると、右パネルから各スロットのステータスを調整できます。
過去日は選択できません。

○　ガイドが自分でシフトを決定する
https://scent-of-asa.onrender.com/staff/guide_login.html
英語ガイド１/２/３、日本語ガイド１/２/３のいずれかを選んでログインしてください。
カレンダーの日付をクリックすると、右パネルから各スロットのステータスを調整できます。（ログインしたガイド（英・日１２３のいずれか）としての入力できます。）
過去日は選択できません。

スタッフサイドの登録が完了すると、Booking（予約）画面で指定した日付が予約可能になります。

○お客さんが登録した予約をスタッフサイドで確認する。
https://scent-of-asa.onrender.com/staff/reservation_detail.html
予約完了画面が表示されると予約した内容が「予約詳細テーブル」に記載されます。
